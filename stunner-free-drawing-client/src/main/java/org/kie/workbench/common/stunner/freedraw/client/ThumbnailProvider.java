...
package org.kie.workbench.common.stunner.freedraw.client;
...
@Dependent
public class ThumbnailProvider extends BindableShapeSetThumbProvider {

    @Inject
    public ThumbnailProvider(final DefinitionManager definitionManager) {
    ...
    protected boolean thumbFor(final Class<?> clazz) {
        return isSameClass(clazz,
                           FreeDrawingSet.class);
    }

    @Override
    public SafeUri getThumbnailUri() {
        return FreeDrawingImageResources.INSTANCE.thumb().getSafeUri();
    }
}
